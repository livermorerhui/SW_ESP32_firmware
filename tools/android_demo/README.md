...
6. After connected, app auto-enables TX notify (CCCD), requests MTU (target 185), and probes `CAP?`.
...
- Typical notify/stream verification path:
  ...
  2. Confirm subsequent `RX-RAW` chunks exist.
  3. Confirm corresponding `RX` complete lines are emitted.
  4. Confirm stream lines (`EVT:STREAM`) appear continuously.
