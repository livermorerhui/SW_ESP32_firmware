...
| FW-OPT-017 | Demo APP Wave control state reducer | 中风险重构 / 控制状态可测性 | 先审后做 | 否 | P3 | Demo APP 控制链小包 | `DemoViewModel.kt`；`WaveLifecycleCommandGate.kt`；`WaveLifecycleCommandGateTest.kt`；`reports/task_20260606_demo_app_refactor_master_plan.md`；本总表 | 只允许先抽纯 state reducer：输入 pending start/stop、wave truth、snapshot merge context、availability，输出 flags / notices / formal session action；不得迁移 `client.send`、truth refresh job、BLE command 时序。需补双向测试：Stop invalidates Start、新 Start 可继续、snapshot false 不清 ready、Stop ack fallback、new formal event 仍触发 |
