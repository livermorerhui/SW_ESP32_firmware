...
| FW-OPT-017 | Demo APP Wave control state reducer | 中风险重构 / 控制状态可测性 | 第一阶段已完成 | 否 | P3 | Demo APP 控制链小包 | `WaveControlStateReducer.kt`；`WaveControlStateReducerTest.kt`；`WaveLifecycleCommandGate.kt`；`WaveLifecycleCommandGateTest.kt`；`DemoStartReadyRegressionTest.kt`；`DemoViewModel.kt`；`reports/task_20260606_demo_app_refactor_master_plan.md`；`reports/task_20260606_demo_app_wave_control_state_reducer.md`；本总表 | 已抽 pure reducer：wave runtime transition、formal wave truth sync、pending flags、snapshot start_ready merge、authoritative wave output、optimistic stop state。保留 `client.send`、truth refresh job、BLE command 时序、`WaveLifecycleCommandGate` token 调用和 test session start/finish 副作用。后续如继续 B4 第二阶段，只能先审 pending lifecycle / formal session action，不得直接迁 command send |
| FW-OPT-018 | Demo APP Motion sampling session owner | 可选重构 / 采样链可测性 | 待条件触发 | 否 | P4 | Demo APP 采样链小包 | `MotionSamplingExporter.kt`；`MotionSamplingSection.kt`；`DemoViewModel.kt`；`reports/task_20260606_demo_app_refactor_master_plan.md`；本总表 | 只有继续扩展 motion sampling 或导出字段时启动。可抽 `MotionSamplingSessionStore`：start/stop/clear、row build、dd/dt/dw/dt、export metadata update；先不改 exporter 和 UI。测试覆盖 start metadata、row delta、stop endedAt、clear gating、export path update |
| FW-OPT-019 | Demo APP UI presentation model 收口 | 可选重构 / Compose 参数瘦身 | 待条件触发 | 否 | P4 | Demo APP UI 小包 | `MainScreen.kt`；`CalibrationToolsSection.kt`；`MotionSamplingSection.kt`；`TelemetryChartSection.kt`；`reports/task_20260606_demo_app_refactor_master_plan.md`；本总表 | 只在 UI 继续膨胀或出现参数错绑时启动。先做 presentation DTO / callbacks 分组，不改视觉、不改 ViewModel 行为、不引入新导航架构。测试以 compile + screenshot/manual smoke 为主 |
