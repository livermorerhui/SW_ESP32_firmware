...
2026-06-07 已完成 `FW-OPT-024 Demo APP legacy UI section cleanup`：删除信息架构改造后已无调用点的旧堆叠首页 section：`DeviceConnectSection`、`ScaleSection`、`StableWeightSection`、`WaveSection`。当前 Demo APP 入口以固定顶部 `型号 / 校准 / 采样 / 运行 / 日志` 和 `WaveControlBottomBar` 为准；连接入口保留在顶栏，型号页不再恢复旧连接状态卡片。本包不改 UI 可见路径、BLE command、ESP32 协议、ViewModel business owner 或状态机行为；仅降低旧入口误接回归风险。本地 compile/test/assemble 通过，轻量真机安装打开 smoke 通过。
...
| FW-OPT-024 | Demo APP legacy UI section cleanup | UI 死代码清理 / 防误接旧入口 | 已完成 / 轻量 UI smoke 已通过 | 否 | P4 | Demo APP UI 小包 | `DeviceConnectSection.kt`；`ScaleSection.kt`；`StableWeightSection.kt`；`WaveSection.kt`；`reports/task_20260607_demo_app_legacy_ui_section_cleanup.md`；本总表 | 已删除信息架构改造后无调用点的旧堆叠首页 section。当前连接入口为顶栏，型号/校准/采样/运行/日志为唯一主入口，运行控制为 `WaveControlBottomBar`。本包不改 UI 可见路径、BLE command、ESP32 协议、ViewModel business owner 或状态机行为；本地 compile/test/assemble 通过，轻量真机安装打开 smoke 通过 |
